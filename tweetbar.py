...
class TweetBar(object):
	...
	def __init__(self,applet):
		...
		self.ev_box.connect("button_press_event", self.on_ev_box_button_press_event)
		
		self.imgPrefs = gtk.Image()
		self.imgPrefs.set_from_stock (gtk.STOCK_PREFERENCES, gtk.ICON_SIZE_BUTTON)
		self.btnPrefs = gtk.Button()
		self.btnPrefs.set_image(self.imgPrefs)
		
		self.imgEnter = gtk.Image()
		self.imgEnter.set_from_stock (gtk.STOCK_OK, gtk.ICON_SIZE_BUTTON)
		self.btnSend = gtk.Button()
		self.btnSend.set_image(self.imgEnter)		
		self.btnSend.connect ("clicked",self.on_btnSend_clicked)
		
		self.txtTweet = gtk.Entry()
		self.txtTweet.set_max_length(140)
		self.txtTweet.set_sensitive(True)
	...
	def on_ev_box_button_press_event(self,widget,event):
		self.txtTweet.grab_focus()
